<x-app-layout>
                ...
                <form action="{{ route('reports.update', $report->id) }}" method="POST" id="reportForm">
                            ...
                            <div>
                                ...
                                <select id="distributor_id" name="distributor_id"
                                    ...
                                    @foreach($distributors as $dist)
                                        <option value="{{ $dist->id }}" {{ $report->distributor_id == $dist->id ? 'selected' : '' }}>{{ $dist->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="account_type" name="account_type"
                                    class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                    required>
                                    <option value="GT" {{ $report->account_type == 'GT' ? 'selected' : '' }}>GT</option>
                                    <option value="MT" {{ $report->account_type == 'MT' ? 'selected' : '' }}>MT</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="channel" name="channel"
                                    class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                    required>
                                    <option value="Direct" {{ $report->channel == 'Direct' ? 'selected' : '' }}>Direct
                                    </option>
                                    <option value="Indirect" {{ $report->channel == 'Indirect' ? 'selected' : '' }}>
                                        Indirect</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select id="province_id" name="province_id"
                                    ...
                                    @foreach($provinces as $prov)
                                        <option value="{{ $prov->id }}" {{ $report->outlet->city->province_id == $prov->id ? 'selected' : '' }}>{{ $prov->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="city_id" name="city_id"
                                    ...
                                    <option value="{{ $report->outlet->city_id }}" selected>
                                        {{ $report->outlet->city->name }}
                                    </option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="outlet_id" name="outlet_id"
                                    ...
                                    <option value="{{ $report->outlet_id }}" selected>{{ $report->outlet->name }}
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="mb-8 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
                        <div
                            ...
                            <span>Bagian C – Produk Terjual</span>
                            ...
                            <table class="w-full text-left text-sm md:text-base" id="itemsTable">
                                <thead class="hidden md:table-header-group">
                                    <tr class="bg-gray-100 dark:bg-gray-700/50 border-b dark:border-gray-700 text-[10px] md:text-xs font-bold uppercase tracking-wider text-gray-500">
                                        ...
                                        <th class="p-3 w-12 text-center">Actions</th>
                                    </tr>
                                </thead>
                                <tbody id="itemRows">
                                    @foreach($reports as $index => $item)
                                        <tr class="flex flex-col md:table-row border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition item-row">
                                            <td class="p-2 md:px-3 align-middle relative">
                                                <div class="md:hidden text-[10px] font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-3 border-b dark:border-gray-700 pb-2">Item Detail</div>
                                                <div class="md:hidden text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Brand & Produk</div>
                                            ...
                                            <td class="p-2 md:px-6 align-middle">
                                            ...
                                            <td class="p-2 md:px-6 {{ auth()->user()->isAdmin() ? 'hidden' : '' }} align-middle">
                                                <div class="md:hidden text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Harga Jual</div>
                                            ...
                                            <td class="p-2 md:px-3 align-middle">
                                                ...
                                                <div class="text-xs md:text-sm text-gray-500 text-right font-medium">
                                                    <span class="het-display text-xs md:text-sm">{{ number_format($item->het, 0, ',', '.') }}</span>
                                                    <input type="hidden" class="het-hidden" value="{{ $item->het }}">
                                                </div>
                                            </td>
                                            <td class="p-2 md:px-3 align-middle">
                                                <div class="md:hidden text-[10px] font-bold text-gray-500 uppercase tracking-wider">Disc</div>
                                                <span class="disc-display font-black text-red-500 text-xs md:text-sm bg-red-50 dark:bg-red-900/20 px-1.5 py-0.5 rounded">{{ number_format($item->discount, 1) }}%</span>
                                            </td>
                                            <td class="p-2 md:px-3 align-middle">
                                                <div class="md:hidden text-[10px] font-bold text-gray-500 uppercase tracking-wider">Subtotal</div>
                                                <span class="row-total font-black text-indigo-600 dark:text-indigo-400 text-sm md:text-base" data-value="{{ $item->total_price }}">Rp {{ number_format($item->total_price, 0, ',', '.') }}</span>
                                            </td>
                                            <td class="p-2 md:px-3 align-middle text-center">
                                                <button type="button" onclick="removeRow(this, {{ $item->id }})"
                        ...
                        <div
                            ...
                            Grand Total: <span id="grandTotal">Rp
                                {{ number_format($reports->sum('total_price'), 0, ',', '.') }}</span>
                        </div>
                    </div>
    ...
    <script>
        ...
        async function fetchProducts(brandSelect) {
            ...
            const selectedId = productSelect.dataset.selectedProduct;
            products.forEach(p => {
                const selected = (selectedId && p.id == selectedId) ? 'selected' : '';
                productSelect.innerHTML += `<option value="${p.id}" ${selected}>${p.name}</option>`;
            });

            if (selectedId) {
                setHET(productSelect);
                delete productSelect.dataset.selectedProduct;
            }
        }
        ...
        function setHET(productSelect) {
            ...
            if (product) {
                ...
                // Only set price if price input was empty (meaning it's likely a change of product)
                // BUT for edit, we might want to keep original price IF product hasn't changed.
                // However, user requirement says price is read-only from master anyway.
        ...
        window.addEventListener('load', () => {
            // Trigger product fetching for initial rows
            document.querySelectorAll('.product-select').forEach(ps => {
                const brandSelect = ps.closest('tr').querySelector('select[name*="brand_id"]');
                if (brandSelect.value) {
                    fetchProducts(brandSelect);
                }
            });
        });
    </script>
