                ...
                <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-8">
                            <span class="px-5 py-2 rounded-lg text-[10px] font-black tracking-widest uppercase {{ $attendance->type === 'check-in' ? 'bg-green-500 text-white shadow-lg shadow-green-100' : 'bg-blue-600 text-white shadow-lg shadow-blue-100' }}">
                                {{ $attendance->type }}
                            </span>
                            <div class="text-right">
                                <span class="block text-xl font-black text-gray-900 dark:text-white">{{ date('d M Y', strtotime($attendance->date)) }}</span>
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Tanggal Absensi</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div class="space-y-6">
                                ...
                                <div>
                                    <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Beauty Advisor</div>
                                    <div class="font-black text-gray-800 dark:text-gray-100 uppercase">{{ $attendance->user->name }}</div>
                                    <div class="text-xs text-gray-500 font-bold mt-0.5">{{ $attendance->user->distributor->name ?? '-' }}</div>
                                </div>
                            </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-50 dark:border-gray-700 bg-gray-50/30">
                            <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.25em]">Dokumentasi Foto</h3>
                        </div>
                        <div class="p-6">
                            <div class="relative group cursor-pointer" onclick="window.open('{{ asset('storage/' . $attendance->photo_path) }}', '_blank')">
                                <div class="aspect-[3/4] rounded-lg overflow-hidden ring-4 ring-white dark:ring-gray-900 shadow-xl">
                                ...
                                <div class="absolute inset-0 bg-indigo-600/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-lg">
                                    <div class="p-3 bg-white/90 rounded-full shadow-2xl transform scale-75 group-hover:scale-100 transition-transform duration-300">
